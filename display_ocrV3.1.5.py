...
INSTALL_DIR = Path(r"C:\Program Files\DisplayOCR")
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # 解像度変更 → clamp、DPI変更 → 右上に再配置
        # 起動直後の誤発火を避けるため 1秒後に接続する
        QTimer.singleShot(1000, self._connect_screen_signals)
    ...
    def setup_tray(self):
        ...
        self._tray_menu.addAction(act_scan)
        self._tray_menu.addSeparator()
        self._tray_menu.addAction(act_quit)

        # setContextMenu は使わず activated で手動制御（Qt の位置バグ回避）
    ...
    def _on_tray_activated(self, reason):
        if reason == QSystemTrayIcon.Trigger:          # 左クリック
            ...
            # QCursor.pos() が最も確実（tray.geometry()はWindows上で不正確）
            pos = QCursor.pos()
            ...
            # マウス位置の左上にメニューを出し、画面外にはみ出ないよう補正
            x = min(pos.x(), screen.right() - mw)
            y = min(pos.y() - mh, screen.bottom() - mh)
            x = max(x, screen.left())
            y = max(y, screen.top())
    ...
    def restore_position(self):
        ...
        if x is not None and y is not None:
            ...
            self.move(screen.right() - w - 20, screen.top() + 20)

    def _connect_screen_signals(self):
        """起動直後の誤発火を避けるため、1秒後にスクリーン変更シグナルを接続する。"""
        for screen in QGuiApplication.screens():
            screen.geometryChanged.connect(lambda _: QTimer.singleShot(300, self._clamp_to_screen))
            screen.logicalDotsPerInchChanged.connect(lambda _: QTimer.singleShot(300, self._reposition_after_dpi_change))
        QGuiApplication.instance().screenAdded.connect(lambda _: QTimer.singleShot(300, self._clamp_to_screen))

    def _clamp_to_screen(self):
        """画面外にはみ出ていたら画面内に収める（DPI変更・解像度変更対応）。"""
        screen = QGuiApplication.primaryScreen().availableGeometry()
        w, h = self.width(), self.height()
    ...
    def _reposition_after_dpi_change(self):
        """DPI変更後は座標系が変わるため、右上基準で再配置する。"""
        screen = QGuiApplication.primaryScreen().availableGeometry()
        # self.width() は非表示中に不正確なため固定サイズを使う
        w = self.sizeHint().width() or 360
        h = self.sizeHint().height() or 330
        x = screen.right() - w - 20
        y = screen.top() + 20
        self.move(x, y)
    ...
    def install_to_program_files(self):
        """DisplayOCR.exe を C:\\Program Files\\DisplayOCR にコピーする。
        成功したらインストール先パス(Path)を返す。失敗したら None を返す。
        Python スクリプト実行中（テスト時）はコピーをスキップしてスクリプトパスを返す。"""

        # ── Python テスト時はファイルコピー不要 ──────────────────────
        if not is_frozen():
            QMessageBox.information(
                self, "自動起動",
                "現在 Python スクリプトで実行中のため、\n"
                "C:\\Program Files\\DisplayOCR へのコピーはスキップします。\n\n"
                "配布用 DisplayOCR.exe で起動すると、\n"
                "自動的にここへコピーされます。"
            )
            # テスト用：.py ファイルを直接指すスタートアップ CMD を作成
            return get_app_launch_target()

        # ── exe 実行時：Program Files にコピー ───────────────────────
        try:
            INSTALL_DIR.mkdir(parents=True, exist_ok=True)
        except PermissionError:
            QMessageBox.warning(
                self, "自動起動",
                "C:\\Program Files\\DisplayOCR フォルダの作成に失敗しました。\n"
                "管理者権限でアプリを起動してから再度お試しください。"
            )
            return None
        except Exception as e:
            QMessageBox.warning(self, "自動起動", f"フォルダの作成に失敗しました。\n{e}")
            return None

        src = Path(sys.executable).resolve()   # 実行中の DisplayOCR.exe
        dst = INSTALL_DIR / "DisplayOCR.exe"   # 固定ファイル名
        try:
            shutil.copy2(str(src), str(dst))
        except PermissionError:
            QMessageBox.warning(
                self, "自動起動",
                "DisplayOCR.exe のコピーに失敗しました（権限エラー）。\n"
                "管理者権限でアプリを起動してから再度お試しください。"
            )
            return None
        except Exception as e:
            QMessageBox.warning(self, "自動起動", f"DisplayOCR.exe のコピーに失敗しました。\n{e}")
            return None

        QMessageBox.information(
            self, "自動起動",
            "アプリを C:\\Program Files\\DisplayOCR フォルダに配置しました。\n"
            "以後自動起動はここから呼び出します。"
        )
        return dst
    ...
    def apply_startup_setting(self, from_ui=False):
        ...
        try:
            if enabled:
                installed_path = self.install_to_program_files()
                if installed_path is None:
                    # インストール失敗 → チェックボックスを元に戻す
                    ...
                    self.settings.save()
                    return
                self.create_startup_cmd(installed_path)
            else:
    ...
    def create_startup_cmd(self, launch_target):
        """スタートアップ CMD を作成する。
        exe 版 : start "" "C:\\Program Files\\DisplayOCR\\DisplayOCR.exe"
        Python  : start "" "python.exe" "スクリプトパス"  (テスト用)
        """
        launch_target = Path(launch_target)
        if is_frozen():
            cmd = f'@echo off\r\nstart "" "{launch_target}"\r\n'
        else:
