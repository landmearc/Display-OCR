...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QSpinBox, QMessageBox, QFileDialog, QLineEdit, QDialog,
...
def _keyboard_input(vk: int, flags: int = 0) -> INPUT:
    return INPUT(type=INPUT_KEYBOARD, union=INPUTUNION(ki=KEYBDINPUT(wVk=vk, wScan=0, dwFlags=flags, time=0, dwExtraInfo=None)))
...
def should_treat_as_chiseki(raw_text: str) -> bool:
    ...
    s = s.translate(str.maketrans({"０": "0", "１": "1", "２": "2", "３": "3", "４": "4", "５": "5", "６": "6", "７": "7", "８": "8", "９": "9", "：": ":"}))
...
class CaptureOverlay(QWidget):
    ...
    def capture_and_ocr(self, local_rect: QRect):
        ...
        t_path = self.settings["tesseract_path"].strip()
        if t_path:
        ...
        try:
            ...
            QMessageBox.warning(self, "OCRエラー", f"OCRに失敗しました。\n\n{e}\n\nTesseractのパスを確認してください。")
            self.canceled.emit()
            return
...
class PasteDialog(QDialog):
    def __init__(self, main_window, current_text="", raw_ocr=""):
        ...
        bottom = QHBoxLayout()
        self.btn_cancel = QPushButton("閉じる")
        bottom.addStretch(1)
    ...
    def paste_and_close(self, text: str, source="manual"):
        text = (text or "").strip()
        if not text:
            return
    ...
    def paste_current(self):
        ...
        self.paste_and_close(text, source="ocr")
    ...
    def paste_history(self):
        ...
        if item:
            self.paste_and_close(item.text(), source="history")

    def paste_master(self):
        item = self.master_list.currentItem()
        if item:
            self.paste_and_close(item.text(), source="master")
...
class MainWindow(QWidget):
    ...
    def build_ui(self):
        ...
        row_t = QHBoxLayout()
        row_t.addWidget(QLabel("Tesseract"))
        self.edit_tess = QLineEdit()
        self.btn_tess = QPushButton("参照")
        row_t.addWidget(self.edit_tess, 1)
        row_t.addWidget(self.btn_tess)
        lay.addLayout(row_t)
        ...
        self.edit_tess.setText(self.settings["tesseract_path"])
        ...
        self.btn_tess.clicked.connect(self.select_tesseract)
        ...
        self.edit_tess.editingFinished.connect(self.save_settings_from_ui)
    ...
    def save_settings_from_ui(self):
        ...
        self.settings["tesseract_path"] = self.edit_tess.text().strip()
        self.settings.save()
        self.history.trim()
        self.history.save()

    def select_tesseract(self):
        path, _ = QFileDialog.getOpenFileName(self, "tesseract.exe を選択", self.edit_tess.text() or r"C:\Program Files\Tesseract-OCR", "実行ファイル (*.exe)")
        if path:
            self.edit_tess.setText(path)
    ...
    def show_paste_dialog(self, current_text: str, raw_ocr: str = ""):
        dlg = PasteDialog(self, current_text=current_text, raw_ocr=raw_ocr)
        result = dlg.exec()
        if self.continuous_mode and self._scanning:
            self.hide()
            QTimer.singleShot(150, self.overlay.begin_capture)
